...
def producer():
    ...
    channel.queue_declare(queue='bank_callback', )
    # 注意在rabbitmq中，消息想要发送给队列，必须经过交换(exchange)，
    # 初学可以使用空字符串交换(exchange='')，
    # 它允许我们精确的指定发送给哪个队列(routing_key=''),
    # 参数body值发送的数据
    data = {"id": 1, "status": "00"}
    channel.basic_publish(exchange='/product', routing_key='channel', body=json.dumps(data, ensure_ascii=False))
    print("已经发送了消息")
...
def consumer():
    ...
    # 5. 接收来自指定queue的消息
    # queue 接收指定queue的消息 on_message_callback 接收到消息后的处理程序 auto_ack 指定为True，表示消息接收到后自动给消息发送方回复确认，已收到消息
